...
class TypeConverterTest < Minitest::Test
  def test_pg_converts_nil
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:string])
  ...
  def test_pg_converts_integers
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:integer])
    assert_equal 42, converter.convert("42", 0)
  end

  def test_pg_converts_float
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:float])
    assert_equal 42.5234123, converter.convert("42.5234123", 0)
  end

  def test_pg_converts_decimal
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:decimal])
    assert_equal BigDecimal.new("42.59"), converter.convert("42.59", 0)
  end

  def test_pg_converts_boolean
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:boolean])
  ...
  def test_pg_converts_date
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:date])
    assert_equal Date.new(2017, 2, 28), converter.convert("2017-02-28", 0)
  end

  def test_pg_converts_datetime
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:datetime])
  ...
  def test_pg_converts_time
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:time])
  ...
  def test_pg_converts_json
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:json])
  ...
  def test_pg_converts_hstore
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:hstore])
  ...
  def test_pg_converts_array
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:array])
  ...
  def test_converts_row_to_hash
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:integer, :string], ["id", "name"])
    row = converter.to_hash ["42", "Rando"]
    assert_equal({"id" => 42, "name" => "Rando"}, row)
  end

  def test_converts_to_array
    converter = MicroRecord::TypeConverter.fetch!('PostgreSQL').new([:integer, :string])
