    ...
    class PolymorphicBelongsTo
      ...
      def run(rows, use_cursor: false, query_logger: nil, measurements: nil)
        query(rows) { |scope|
          ...
          q = Query.new(scope, use: @use, eager_loaders: eager_loaders, query_logger: query_logger, measurements: measurements, active_record_fallback: @active_record_fallback)
          assoc_rows = use_cursor ? q.find_each_with_cursor.to_a : q.to_a
